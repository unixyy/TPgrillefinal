...
public class Instance {
  ...
  public boolean estValide(Solution s) {
    ...
    if (s.stream().anyMatch(Objects::isNull))
      return false;
  ...
  private List<Coord> closestPieces(Coord x, List<Coord> pieces, List<Coord> path) {
    ...
    if (path.size() == pieces.size() || pieces.isEmpty())
      return path;
  ...
  public ArrayList<Integer> greedyPermut() {
    ...
    ArrayList<Coord> pieces = (ArrayList<Coord>) listeCoordPieces.clone();

    return closestPieces(startingP, pieces, new ArrayList<>())
        .stream()
        .map(x -> listeCoordPieces.indexOf(x))
        .collect(Collectors.toCollection(ArrayList::new));
  }

  public Stream<Coord> getPathBetween(Coord start, Coord end) {
    return Stream.iterate(start, coord -> !coord.equals(end), coord -> coord.nextTo(end));
  }
  ...
  public Solution calculerSol(ArrayList<Integer> permut) {
    ...
    Stream<Coord> piecesPath = Stream
        .iterate(0, n -> n < permut.size() - 1, n -> n + 1)
        .flatMap(i -> getPathBetween(getPiece.apply(i), getPiece.apply(i + 1)));

    Stream<Coord> completePath = Stream.concat(getPathBetween(startingP, getPiece.apply(0)), piecesPath);

    Solution res = new Solution();
    res.addAll(completePath.limit(k + 1).collect(Collectors.toList()));
